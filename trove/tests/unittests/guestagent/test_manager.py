...
class GuestAgentCassandraDBManagerTest(testtools.TestCase):
    ...
    def _prepare_dynamic(self, packages,
        ...
        PKG_COUNT = 1 if packages else 0
        COUNT = 1 if device_path else 0
        SEC_COUNT = 1 if is_db_installed else 0
        migrate_count = 1 * COUNT * PKG_COUNT if not backup_id else 0

        mock_status = mock()
        self.manager.appStatus = mock_status
        when(mock_status).begin_install().thenReturn(None)
        when(mock_status).mark_failed().thenReturn(None)
        ...
        # verification/assertion
        verify(mock_status).begin_install()
        verify(volume.VolumeDevice, times=(COUNT * PKG_COUNT)).format()
        verify(volume.VolumeDevice,
               times=(migrate_count * SEC_COUNT)).migrate_data(any())
        verify(mock_app, times=PKG_COUNT).install_if_needed(packages)
        verify(mock_status, times=(1 - PKG_COUNT)).mark_failed()
