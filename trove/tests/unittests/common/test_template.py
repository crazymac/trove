...
class TemplateTest(testtools.TestCase):
    ...
    def test_render_mysql_logging_success(self):
        template.CONF.get('mysql').allow_database_logging = True
        config_true = template.SingleInstanceConfigTemplate(
            'mysql', self.flavor_dict, self.server_id)
        rendered_true = config_true.render()
        self.assertIn('general_log_file', rendered_true)
        self.assertIn('log_slow_queries', rendered_true)
        self.assertIn('log-error', rendered_true)

    def test_render_redis_logging_success(self):
        template.CONF.get('redis').allow_database_logging = True
        config_true = template.SingleInstanceConfigTemplate(
            'redis', self.flavor_dict, self.server_id)
        rendered_true = config_true.render()
        self.assertIn('logfile', rendered_true)

    def test_render_mysql_logging_success_no_logs(self):
        template.CONF.get('mysql').allow_database_logging = False
        config_true = template.SingleInstanceConfigTemplate(
            'mysql', self.flavor_dict, self.server_id)

        rendered_false = config_true.render()

        self.assertNotIn('general_log_file', rendered_false)
        self.assertNotIn('log_slow_queries', rendered_false)
        self.assertNotIn('log-error', rendered_false)

    def test_render_redis_logging_success_no_logs(self):
        template.CONF.get('redis').allow_database_logging = False
        config_true = template.SingleInstanceConfigTemplate(
            'redis', self.flavor_dict, self.server_id)
        rendered_false = config_true.render()
        self.assertNotIn('logfile', rendered_false)
