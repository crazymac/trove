...
GROUP = "dbaas.api.clustering.mongodb"
DATASTORE_NAME = "mongodb"
CLUSTER_PROVISIONING_TIMEOUT = 2700


@test(groups=[GROUP])
class CreateMongoDBCluster(base.CreateClusterBase):
    ...
    def get_datastore_or_skip_test(self):
        if not self._check_if_datastore_exist(DATASTORE_NAME):
            raise SkipTest("Please run kick-start for MongoDB datastore.")
        return self._get_datastore_and_its_version(DATASTORE_NAME)

    @test
    def test_mongodb_datastore_and_its_version_for_existance(self):
    ...
    @test(depends_on=[
        test_mongodb_datastore_and_its_version_for_existance])
    ...
    def test_create_cluster_successfuly(self):
        ...
        self.cluster = self.rd_client.clusters.create(
            ...
            version.id, instances=self.valid_valid_request_body)
